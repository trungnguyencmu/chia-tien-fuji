...
interface GuestPhotosProps {
  tripId: string;
  images: GuestTripImage[];
  onImagesChanged: () => void;
}

export function GuestPhotos({ tripId, images, onImagesChanged }: GuestPhotosProps) {
  const { t } = useLanguage();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [dragOver, setDragOver] = useState(false);
  const [lightboxImage, setLightboxImage] = useState<GuestTripImage | null>(null);
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      const { uploadUrl, imageId } = await getGuestImageUploadUrl(
        tripId,
        file.name,
        file.type,
        file.size,
  ...
  const handleDelete = async (imageId: string) => {
    ...
    try {
      await deleteGuestImage(tripId, imageId);
